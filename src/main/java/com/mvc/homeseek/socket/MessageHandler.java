...
public class MessageHandler extends TextWebSocketHandler {
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		if (StringUtils.isNotEmpty(msg)) {
			...
			logger.info("\n userSessionMap에 (현재 로그인되어있는 아이디)담겨있는 애 : " + userSessionMap.get(message_reid));
			 ...
			 * 
			 * if (strs != null && strs.length == 3) { String cmd = strs[0]; String
			 * message_senid = strs[1]; String message_reid = strs[2];
			 * 
			 * // 작성자가 로그인해있다면 // 쪽지 받는 사람 << WebSocketSession boardWriterSession =
			 ...
			 * 
			 * TextMessage tmpMsg = new TextMessage( message_senid + "님이 " + message_reid +
			 * "님에게 쪽지를 보냈습니다."); boardWriterSession.sendMessage(tmpMsg); // 이 부분이 쪽지를 보낸
			 * 사람의 id에게 send해주는 부분? } } } }
			 */
		}
	}
